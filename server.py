...
async def get_symbols_for_exchanges(exchanges: List[str]) -> List[str]:
    ...
    try:
        ...
        for ex in exchanges:
            if ex == "Ondo":
                ...
                exchange_symbols.append(set(o_map.keys()))
                ...
                v_syms = variational.client.get_symbols()
        ...
        if exchange_symbols:
            common = set.intersection(*exchange_symbols) if len(exchange_symbols) > 1 else exchange_symbols[0]
...
async def api_symbols():
    ...
    try:
        ...
        # Get symbols from active exchanges: Ondo, RH_Lighter, Variational
        for ex in ["Ondo", "RH_Lighter", "Variational"]:
...
async def get_funding_rate(ex: str, sym: str) -> float:
    ...
    if ex == "RH_Lighter":
        return rh_lighter_ws.client.get_funding(sym)
    elif ex == "Variational":
        return variational.client.get_funding(sym)
    elif ex == "Ondo":
        return await get_ondo_funding(sym)
    return 0.0
...
async def history_logger_loop():
    ...
    while True:
        try:
            ...
            pairs = [("Ondo", "RH_Lighter"), ("Ondo", "Variational"), ("Variational", "RH_Lighter")]
...
@app.get("/api/history")
async def api_history(symbol: Optional[str] = None, limit: int = 50):
    try:
        conn = sqlite3.connect(DB_PATH)
        cur = conn.cursor()
        if symbol:
...
async def api_scan_top(long_ex: str, short_ex: str, min_spread: float = -100.0):
    syms = await get_symbols_for_exchanges([long_ex, short_ex])
    
    async def check_sym(s):
        lb, la = await get_price(long_ex, s)
        sb, sa = await get_price(short_ex, s)
        if la > 0 and sb > 0:
            spr = (sb - la) / la * 100.0
            if spr >= min_spread:
                l_fr = await get_funding_rate(long_ex, s)
                s_fr = await get_funding_rate(short_ex, s)
                return {
                    "symbol": s,
    ...
    for i in range(0, len(syms), chunk_size):
        ...
        chunk_res = await asyncio.gather(*(check_sym(s) for s in chunk))
